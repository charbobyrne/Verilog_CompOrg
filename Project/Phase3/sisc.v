...
module sisc (clk, rst_f);
  ...
  // reads and writes using mm_addr, writes regb data when dm_we is high
  dm u11 (
    mm_addr,
    mm_addr,
    regb,
  ...
  // mux4 selects which register to read for port B
  mux4 u12 (
    instr[15:12],
    instr[23:20],
    rb_sel,
  ...
  // mux16 selects the memory address source
  mux16 u13 (
    instr[15:0],
    rega + instr[15:0],
    mm_sel,
  ...
  initial
    $monitor(
      "t=%0t PC=%h IR=%h R1=%h R2=%h R3=%h R4=%h R5=%h STAT=%h ALU_OP=%h WB_SEL=%b RF_WE=%b PC_SEL=%b PC_WRITE=%b IR_LOAD=%b BR_SEL=%b WD=%h MM_SEL=%b DM_WE=%b RB_SEL=%b",
      $time,
      pc_out,
      instr,
      u2.ram_array[1],
